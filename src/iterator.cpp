...
struct UmapFactory {
  ...
  void create_impl(const Gradient &gradient, bool pcg_rand, bool batch) {
    if (pcg_rand) {
      create_impl<typename BatchRngFactory::PcgFactoryType, DoMove>(gradient, batch);
    } else {
      create_impl<typename BatchRngFactory::TauFactoryType, DoMove>(gradient, batch);
    }
  }
  ...
  void create_impl(Worker &worker, const Gradient &gradient, Progress &progress, Parallel &parallel) {
...
void create_umap(UmapFactory &umap_factory, List method_args) {
  ...
  Rcerr << "a = " << a << ", b = " << b << ", gamma = " << gamma << "\n";
  if (approx_pow) {
    const uwot::apumap_gradient gradient(a, b, gamma);
    umap_factory.create(gradient);
  } else {
    const uwot::umap_gradient gradient(a, b, gamma);
    // Rcerr << gradient.grad_attr(1, 2, 3);
    umap_factory.create(gradient);
    for (int t=0;t<10;t++){
      Rcout << gradient.grad_attr(t/10+1, 100, 3) << "\t";
      Rcout << gradient.grad_rep(t/10+1, 100, 3) << "\t";
      Rcout << gradient.clamp_grad(t) << "\n";
    }
  }
}
...
void create_snnmap(UmapFactory &umap_factory, List method_args) {
  std::vector<std::string> arg_names = {"ai", "bi", "ndim"};
  validate_args(method_args, arg_names);
  
  std::vector<float> ai = method_args["ai"];
  std::vector<float> bi = method_args["bi"];
  std::size_t ndim = method_args["ndim"];
  const uwot::snnmap_gradient gradient(ai, bi, ndim);
  umap_factory.create(gradient);
    // for (int t=0;t<10;t++){
    //   Rcout << gradient.grad_attr(t/10+1, 1, 1) << "\t";
    //   Rcout << gradient.grad_rep(t/10+1, 1, 1) << "\n";
    // }
}
...
void create_umapai(UmapFactory &umap_factory, List method_args) {
  ...
  umap_factory.create(gradient);
  for (int t=0;t<10;t++){
    Rcout << gradient.grad_attr(10, t, 1) << "\t";
    Rcout << gradient.grad_rep(10, t, 1) << "\n";
  }
}
...
NumericMatrix optimize_layout_r(
  ...
  const std::size_t ndim = head_embedding.size() / n_head_vertices;
  uwot::EpochCallback *uwot_ecb = create_callback(epoch_callback, ndim, move_other);
